<?php 
	...
	// membuat pagination
	$hitngTtlData 	= kueri("COUNT(*) AS ttl", "peminjaman WHERE status = '2' ");
	$fetchData 			= $hitngTtlData->fetch_array();
	$jmlData		= $fetchData['ttl'];
	$jmlPerhlmn		= 10;
	$rumusPage 		= ceil($jmlData / $jmlPerhlmn);
	$halmnAktif 	= (isset($_GET['hal_ke'])? $_GET['hal_ke']: "1");
...
<?php 
	...
	$pj 	= $conx->query("SELECT * FROM peminjaman 
							...
							WHERE peminjaman.status = '2' ORDER BY tgl_kembali DESC LIMIT $awalData, $jmlPerhlmn");
...
<tbody>
	...
	<tr>
		<td><?= $row['id_pinjam'] ?></td>
		<td><a class='sidenav-trigger' data-target='tbl_kmb_<?= $row['id_akun'] ?>'><?= $row['nm_pengguna'] ?></a></td>
		<td><a class='sidenav-trigger' data-target='<?= $row['id_brg'] ?>'><?= $row['nm_brg'] ?></a></td>
		<td><?= $row['tgl_masuk'] ?></td>
...
<div id="tbl_kmb_<?= $row['id_akun'] ?>" class="sidenav r-detail-pmjm">
...
<div id="<?= $row['id_brg'] ?>" class="sidenav r-detail-pmjm">
	<div class="card-panel">
...
<?php if ($halmnAktif > 1) { ?>
 	...
 	<li class="waves-effect"><a href="?hal_ke=<?= $halmnAktif - 1 ?>"><i class="material-icons">chevron_left</i></a></li>
...
?>
 	...
 	<li class="active"><a href="?hal_ke=<?= $p ?>"><?= $p ?></a></li>

<?php
		else :
?>
 	<li class="waves-effect"><a href="?hal_ke=<?= $p ?>"><?= $p ?></a></li>
...
<?php if ($halmnAktif < $rumusPage) { ?>
 	...
 	<li class="waves-effect"><a href="?hal_ke=<?= $halmnAktif + 1 ?>"><i class="material-icons">chevron_right</i></a></li>
