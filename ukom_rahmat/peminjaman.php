...
<div id="data">
 	...
 	<?php 
		...
		$sql 	= $conx->query("SELECT * FROM peminjaman
								...
								INNER JOIN akun_pengguna ON peminjaman.id_akun = akun_pengguna.id_akun
								 WHERE peminjaman.id_akun = '$id' $dtBrg ORDER BY tgl_masuk DESC");
	...
	?>
		...
		<div class="card-panel r-box">
				...
				switch ($stat = $row['status']) {
					...
					default:
						$stat = "
									<a class='waves-effect waves-light btn r-delete batal' href='../ukom_rahmat/batal_pinjam.php?id=" . $row['id_pinjam'] . "'>Batal</a>";
	...
	<?php
 	 ...
 	 ?>
 	 	...
 	 	<div class="col l12 card-panel center"><p>Tidak ada barang yang dipesan atau dipinjam</p></div>
...
<script>
	...
	$(document).ready(function() {
		$('#searchBox').on('keyup', function() {
			$('#data').load('ls_peminjaman.php?keyword=' + $('#searchBox').val());
		});
	});
</script>
